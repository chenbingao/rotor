...
pub struct Metrics {
  ...
  /// Number of expired callbacks waiting to be drained.
...
impl<T: Send + 'static> TimingWheel<T> {
  ...
  /// # let (wheel, _guard) = TimingWheel::start(WheelConfig::default(), |_: String| {});
  ...
  /// Number of expired callbacks waiting to execute.
  #[inline]
...
///
/// Note: `task.abort()` waits for the next `.await` point.  A blocking
/// synchronous callback (infinite loop, long sleep, etc.) will keep the
/// worker alive until it returns.  Keep callbacks fast and non-blocking.
...
impl<T: Eq + Hash + Clone + Send + 'static> TimingWheel<T> {
  ...
  ///
  /// The `callback` closure is invoked synchronously in the worker for each
  /// expired task.  Keep callbacks fast; for I/O work, send the id through
  /// a channel to a dedicated async task.
  ///
  /// # Example
  ///
  /// ```rust,no_run
  ...
  /// ```
  pub fn start<F>(config: WheelConfig, callback: F) -> (TimingWheel<T>, TimingWheelGuard)
  where
