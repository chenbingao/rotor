...
async fn test_metrics() {
  pause();
  let (h, _g) = TimingWheel::start(cfg_fast(), |_: String| {});
  sleep(Duration::from_millis(5)).await;
...
async fn test_panicked_callback_increments_abnormal() {
  pause();
  let (h, _g) = TimingWheel::start(cfg_fast(), move |_: String| panic!("intentional"));
  sleep(Duration::from_millis(5)).await;
  h.insert("x".into(), Duration::from_millis(100));
...
/// Channel 满时 insert/remove/reset 返回 false 且 dropped_total 递增，
/// 已调度的任务到期后正常触发（cancelled 集合无残留）。
#[tokio::test]
async fn test_full_channel_ops_return_false_no_leak() {
  ...
  // 插入一个长延迟任务确保 worker 已接收 cmd
  assert!(h.insert("a".into(), Duration::from_secs(60)));
  sleep(Duration::from_millis(10)).await;

  // 填满 channel（1 槽位）
  let mut ok = true;
  while ok {
    ok = h.insert("filler".into(), Duration::from_secs(60));
  }

  // channel 满时三个操作均返回 false
  let d0 = h.dropped_total();
  assert!(!h.remove(&"x".to_string()));
  assert_eq!(h.dropped_total(), d0 + 1);

  let d1 = h.dropped_total();
  assert!(!h.reset("y".into(), Duration::from_secs(1)));
  assert_eq!(h.dropped_total(), d1 + 1);

  let d2 = h.dropped_total();
  assert!(!h.insert("z".into(), Duration::from_secs(1)));
  assert_eq!(h.dropped_total(), d2 + 1);

  // 已调度任务的 expired 计数不受影响
  let before_expired = h.expirations_total();

  // 再插入几条到期任务，验证正常触发
  advance(Duration::from_millis(200)).await;
  sleep(Duration::from_millis(50)).await;

  assert!(h.insert("fire1".into(), Duration::from_millis(100)));
  sleep(Duration::from_millis(10)).await;
  assert!(h.insert("fire2".into(), Duration::from_millis(100)));
  sleep(Duration::from_millis(10)).await;

  advance(Duration::from_millis(300)).await;
  sleep(Duration::from_millis(100)).await;
  assert!(h.expirations_total() > before_expired);
...
async fn test_failed_insert_increments_dropped() {
  ...
  let (h, _g) = TimingWheel::start(config, |_: String| {});
