...
async fn stress_50k_one_shot() {
  ...
  let c = Arc::clone(&n);
  let (h, _g) = TimingWheel::start(cfg(), {
    let c = Arc::clone(&c);
    move |_: usize| {
  ...
  let total = 50_000;
  for i in 0..total {
    h.insert(i, Duration::from_secs(5));
  }
  ...
  println!(
    "expired {fired}/{total} ({:.1}%)",
    fired as f64 / total as f64 * 100.0
  );
...
async fn stress_10k_heartbeat() {
  ...
  for round in 0..10 {
    ...
    for i in 0..total {
      h.reset(i, Duration::from_secs(60));
    }
...
async fn stress_throughput() {
  let (h, _g) = TimingWheel::start(cfg(), |_: usize| {});
  let count = 50_000;
  let mut dropped = 0;
  for i in 0..count {
    if !h.insert(i, Duration::from_secs(30)) {
      dropped += 1;
    }
  }
...
async fn stress_long_running() {
  ...
  while start.elapsed() < dur {
    for _ in 0..1000 {
      h.insert(id, Duration::from_secs(30));
      id += 1;
    }
    for i in id.saturating_sub(5000)..id {
      h.reset(i, Duration::from_secs(30));
    }
    for i in id.saturating_sub(10_000)..id.saturating_sub(8000) {
      h.remove(&i);
    }
...
async fn stress_3_hour_delay() {
  ...
  assert_eq!(
    ...
    0,
    "3h task should not fire immediately"
  );
  h.shutdown();
}
...
async fn stress_concurrent_mixed() {
  ...
  let t0 = tokio::spawn(async move {
    for i in 0..total / 4 {
      h0.insert(i, Duration::from_secs(2));
    }
  });
  let t1 = tokio::spawn(async move {
    for i in total / 4..total / 2 {
      h1.insert(i, Duration::from_secs(2));
    }
  });
  let t2 = tokio::spawn(async move {
    for i in total / 2..3 * total / 4 {
      h2.insert(i, Duration::from_secs(2));
    }
  });
  let reset_h = h3.clone();
  let t3 = tokio::spawn(async move {
    for i in 3 * total / 4..total {
      h3.insert(i, Duration::from_secs(2));
    }
  });
...
async fn stress_callback_panic_resilience() {
  ...
  let total = 5000;
  for i in 0..total {
    h.insert(i, Duration::from_secs(2));
  }
  ...
  assert!(
    ok as f64 > total as f64 * 0.85,
    "only {ok}/{total} succeeded after panics"
  );
  h.shutdown();
}
...
async fn stress_sustained_heartbeat() {
  ...
  let n = 5000;
  for i in 0..n {
    h.insert(i, Duration::from_secs(10));
  }
...
async fn stress_worker_panic_recovery() {
  ...
  assert!(
    h.abnormal_total() >= 1,
    "abnormal counter counts panicked callbacks"
  );
  h.shutdown();
}
