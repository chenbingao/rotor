use std::{collections::HashMap, hash::Hash, sync::atomic::Ordering, time::Duration};
...
impl<T: Eq + Hash + Clone> Wheel<T> {
  pub(crate) fn new(tick_interval: Duration) -> Self {
    assert!(
      tick_interval >= Duration::from_millis(1),
      "tick_interval must be >= 1ms"
    );
    Self {
      levels: [
  ...
  pub(crate) fn alloc(&mut self, id: T, expire_tick: u64) -> usize {
    let idx = if let Some(i) = self.free.pop() {
      self.arena[i] = TaskEntry {
        id: id.clone(),
        expire_tick,
      };
      i
    } else {
      self.arena.push(TaskEntry {
        id: id.clone(),
        expire_tick,
      });
      self.arena.len() - 1
  ...
  pub(crate) fn push_to_level(&mut self, expire_tick: u64, idx: usize) {
    ...
    let target_level = if expire_tick >= self.current_tick + self.levels[2].tick_per_advance {
      2
    } else if expire_tick >= self.current_tick + self.levels[1].tick_per_advance {
      1
    } else {
      0
    };
  ...
  pub(crate) fn schedule(&mut self, id: T, timeout: Duration) {
    ...
    let idx = self.alloc(id, expire_tick);
    self.push_to_level(expire_tick, idx);
  }

  pub(crate) fn remove(&mut self, id: &T) {
    // Don't free the arena slot — stale bucket references still hold this idx.
    // process_slot will free it when the old bucket drains and id_map lookup fails.
  ...
  /// Drain all buckets at once, expiring every entry.
  /// Used during shutdown to avoid tick-by-tick iteration.
  pub(crate) fn drain_all(&mut self) -> Vec<T> {
    let saved = self.current_tick;
    self.current_tick = u64::MAX;
    let mut expired = Vec::new();
    for level in (0..=2).rev() {
      for slot in 0..self.levels[level].slots.len() {
        let indices = std::mem::take(&mut self.levels[level].slots[slot]);
        self.process_slot(indices, level, &mut expired);
      }
    }
    self.current_tick = saved;
    expired
  }
  ...
  fn process_slot(&mut self, indices: Vec<usize>, level: usize, expired: &mut Vec<T>) {
    for idx in indices {
      ...
      if *current_info != idx {
        // stale copy — id_map points to a different arena slot (e.g. after a reset)
        self.free.push(idx);
        continue;
      }
